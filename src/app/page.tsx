...
export default function Home() {
  ...
  }, [systemState, addCaanMessage, addSystemMessage, addLogEntry]);
  ...
  }, [pendingMutation, systemState.saturation, runCoherenceGate, applyMutation, addCaanMessage, addSystemMessage, addLogEntry]);
  ...
  const handleSendMessage = useCallback(async (content: string) => {
    ...
    if (currentState.setupComplete && pendingMutation) {
      ...
      if (lowerContent === 'no' || lowerContent === 'reject' || lowerContent === 'cancel' || lowerContent === 'abort' || lowerContent === 'deny') {
        setMessages((prev) => [...prev, createMessage('operator', content)]);
  ...
  }, [isLoading, systemState, messages, pendingMutation, scannedFiles, advanceSetup, addCaanMessage, addLogEntry, addSystemMessage, handleMutationDecision]);
 ...
 addCaanMessage(`Available files:\n${fileList}\n\nTell me which file to target, OPERATOR. Type a number (1-${scannedFiles.length}) or a file path.`);
      ...
      default:
        addCaanMessage(`Unknown action. Available: SCAN, ANALYZE, PROPOSE, HEALTH, SATURATION, DEBATE.`);
    }
  }, [systemState, isLoading, scannedFiles, pendingMutation, mutationsApplied, addCaanMessage, addLogEntry, addSystemMessage]);
        ...
        <div className="w-full lg:w-[480px] flex-shrink-0 flex flex-col min-h-0">
          <div className="flex-1 flex flex-col min-h-0">
            ...
            {systemState.setupComplete && (
              <QuickActions onAction={handleQuickAction} disabled={isLoading} pushStatus={pushStatus} />
            )}
          </div>
        </div>
