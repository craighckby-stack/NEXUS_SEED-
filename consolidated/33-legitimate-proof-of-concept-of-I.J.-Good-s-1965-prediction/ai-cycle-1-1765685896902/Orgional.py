...
def initialize_knowledge_base():
    """Initialize the knowledge base directory and required files."""
    os.makedirs('/content/knowledge_base', exist_ok=True)
    return '/content/knowledge_base'
...
# Configure generative AI with error handling
def configure_generative_ai(api_key):
    ...
    try:
        ...
        raise ImportError("google-generativeai package not found. Please install it first.")
    except Exception as e:
        raise Exception(f"Failed to configure generative AI: {str(e)}")

# Check available models with error handling
def get_available_models(genai):
    """Fetch and validate available generative models."""
    try:
        available_models = []
...
def select_model(available_models):
    """Select the latest available model or fallback to the first one."""
    model_name = 'gemini-2.0-flash-latest'
    return model_name if model_name in available_models else available_models[0]

# Initialize the generative model
def initialize_model(genai, model_name):
    """Initialize the generative model with the selected model name."""
    try:
        return genai.GenerativeModel(model_name)
    except Exception as e:
        raise Exception(f"Failed to initialize model {model_name}: {str(e)}")

# Knowledge base class with improved error handling and validation
class KnowledgeBase:
    ...
    def __init__(self, base_path='/content/knowledge_base'):
        ...
        self.learnings = self._load_json(self.learnings_file, self._default_learnings)
        self.improvement_patterns = self._load_pickle(self.improvement_patterns_file, self._default_patterns)
        self.session_history = self._load_json(self.session_history_file, self._default_history)

        # Ensure directories exist
        os.makedirs(self.base_path, exist_ok=True)

    def _initialize_default_structures(self):
        """Initialize default data structures for the knowledge base."""
        self._default_learnings = {
            'successful_patterns': [],
            'failed_attempts': [],
            'performance_metrics': {},
            'evolution_timeline': [],
    ...
    def _load_pickle(self, path, default=None):
        ...
        try:
            ...
            print(f"Error loading pickle from {path}: {str(e)}")
            return default if default is not None else {}
    ...
    def _save_pickle(self, path, data):
        ...
        try:
            ...
            print(f"Error saving pickle to {path}: {str(e)}")

    def record_improvement(self, cycle, old_code, new_code, metrics):
        """Record a successful improvement with validation."""
...
# Main execution with proper error handling and validation
def main():
    try:
        # Initialize knowledge base
        kb_path = initialize_knowledge_base()
        print(f"Knowledge base initialized at {kb_path}")

        # Load API key
        api_key = load_api_key()

        # Configure generative AI
        genai = configure_generative_ai(api_key)

        # Get available models
        available_models = get_available_models(genai)
        ...
        # Initialize model
        model = initialize_model(genai, model_name)
...
## Evolution Timeline''')
        ...
        if continue_option == 'y':
            try:
                ...
                if not 1 <= additional_cycles <= 10:
                    print("Invalid number of cycles. Using default 3.")
                    additional_cycles = 3
            except ValueError:
                print("Invalid input. Using default 3 cycles.")
                additional_cycles = 3
