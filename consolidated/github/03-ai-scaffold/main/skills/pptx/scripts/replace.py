...
def clear_paragraph_bullets(paragraph: object) -> object:
    """Clear bullet formatting from a paragraph."""
    paragraph_element = paragraph._element.get_or_add_pPr()
...
def apply_paragraph_properties(paragraph: object, para_data: Dict[str, Any]) -> None:
    """Apply formatting properties to a paragraph."""
    text = para_data.get("text", "")

    paragraph_element = clear_paragraph_bullets(paragraph)
    ...
    if para_data.get("bullet", False):
        ...
        paragraph_element.attrib["marL"] = str(level_indent_emu)
        paragraph_element.attrib["indent"] = str(hanging_indent_emu)

        buChar = OxmlElement("a:buChar")
        buChar.set("char", "•")
        paragraph_element.append(buChar)

        if "alignment" not in para_data:
            paragraph.alignment = PP_ALIGN.LEFT
    else:
        paragraph_element.attrib["marL"] = "0"
        paragraph_element.attrib["indent"] = "0"

        buNone = OxmlElement("a:buNone")
        paragraph_element.insert(0, buNone)
    ...
    if "alignment" in para_data:
        ...
        paragraph.alignment = alignment_map.get(para_data["alignment"])
...
def apply_font_properties(run: object, para_data: Dict[str, Any]) -> None:
...
def check_duplicate_keys(pairs: List[tuple]) -> Dict[str, Any]:
...
def apply_replacements(pptx_file: str, json_file: str, output_file: str) -> None:
    ...
    for slide_key, shapes_dict in inventory.items():
        ...
        for shape_key, shape_data in shapes_dict.items():
            ...
            text_frame = shape_data.shape.text_frame  # type: ignore
            ...
            for para_data in replacement_shape_data["paragraphs"]:
