...
def parse_slide_sequence(sequence: str) -> List[int]:
    """Parse a comma-separated sequence of slide indices."""
    try:
        return [int(x.strip()) for x in sequence.split(",")]
    except ValueError:
...
def duplicate_slide(pres, index):
    ...
    # Clear placeholder shapes to avoid duplicates
        ...
        for blip in blips:
            ...
            if old_rId in image_rels:
                new_rId = new_slide.part.rels.get_or_add(
                    image_rels[old_rId].reltype, image_rels[old_rId]._target
                )
                blip.set("{http://schemas.openxmlformats.org/officeDocument/2006/relationships}embed", new_rId)

    # Copy any additional image/media relationships
