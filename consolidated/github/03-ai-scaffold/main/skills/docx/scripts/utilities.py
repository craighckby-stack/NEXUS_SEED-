...
class XMLEditor:
    def __init__(self, xml_path):
        ...
        parser = create_line_tracking_parser()
    ...
    def get_node(
        ...
        if not matches:
            ...
            base_msg = f"Node not found: <{tag}> {' '.join(filters)}".strip()
    ...
    def _get_element_text(self, elem):
        ...
        Returns:
            Concatenated text from all non-whitespace text nodes within the element.
        """
        text_parts = []
    ...
    def replace_node(self, elem, new_content):
        ...
        nodes = parse_fragment(new_content)
        for node in nodes:
    ...
    def insert_after(self, elem, xml_content):
        ...
        nodes = parse_fragment(xml_content)
        for node in nodes:
            if next_sibling:
...
def create_line_tracking_parser() -> defusedxml.sax.xmlreader.XMLReader:
    """
    ...
    def start_elementNS(name, tagName, attrs):
        orig_start_cb(name, tagName, attrs)
        cur_elem = handler.elementStack[-1]
        cur_elem.parse_position = (
            parser._parser.CurrentLineNumber,
            parser._parser.CurrentColumnNumber,
        )

    handler = defusedxml.sax.make_parser().getundi()
    orig_start_cb = handler.startElementNS
    handler.startElementNS = start_elementNS  # type: ignore
    parser = defusedxml.sax.make_parser()
    parser.setContentHandler(handler)
    return parser

def parse_fragment(xml_content):
    """
    ...
    root_elem = defusedxml.minidom.parseString(f"<root>{xml_content}</root>").documentElement
    namespaces = []
    ...
    nodes = [
        defusedxml.minidom.Document().importNode(child, deep=True)
