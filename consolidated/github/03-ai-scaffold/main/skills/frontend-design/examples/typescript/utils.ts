...
export function formatFileSize(bytes: number, decimals: number = 2): string {
  ...
  const k = 1024;
  const dm = Math.max(0, Math.floor(Math.log10(bytes) / Math.log10(k)));
...
export function throttle<T extends (...args: any[]) => any>(
  ...
  return function executedFunction(...args: Parameters<T>) {
    if (!inThrottle) {
      func(...args);
      inThrottle = true;
      // Use `setInterval` for a more precise control
...
export function generateId(length: number = 8): string {
  const id = Array.from({ length }, () => Math.random().toString(36)).join('');
  return id.substring(2, length + 2);
}
