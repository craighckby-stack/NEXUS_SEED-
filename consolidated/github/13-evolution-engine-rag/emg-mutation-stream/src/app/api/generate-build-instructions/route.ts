...
interface ParsedProjectData {
  ...
  techStack: string; // Raw string input
  files: File[];
}
...
// --- Internal Utilities & Service Logic ---

/**
 * Normalizes tech stack string into a clean array of strings.
 */
const normalizeTechStack = (techStack: string): string[] => 
  techStack.split(',')
           .map(s => s.trim())
           .filter(s => s);

/**
 * Parses form data and validates essential fields.
 */
async function parseAndValidateFormData(request: NextRequest): Promise<ParsedProjectData> {
  ...
  if (typeof projectType !== 'string' || !projectType.trim() ||
    ...
    throw new Error('Missing or invalid project specification fields.');
  }

  // Type assertion for files array
  const fileObjects = files.filter(item => item instanceof File) as File[];
...
/**
 * Validates user existence and essential configuration (GitHub token).
 */
async function validateUserAndConfig() {
  ...
  return { user, githubToken: config.githubToken };
}

/**
 * Saves uploaded files to disk and records them in the database.
 */
async function handleFileUploads(projectId: string, files: File[]): Promise<UploadedFileInfo[]> {
...
/**
 * Generates the system and user prompts for the LLM based on project context.
 */
function generatePrompt(
    ...
    const fileReferences = uploadedFilesInfo.map(file =>
...
## User Uploaded Files
${uploadedFilesInfo.length > 0 ? fileReferences : 'No files uploaded'}
...
/**
 * Provides a basic, standardized set of instructions if the LLM call fails.
 */
...
/**
 * Generates build instructions using the LLM, with fallback mechanism.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { projectType, projectDescription, techStack, files } = await parseAndValidateFormData(request);

    // 2. Validate User & Configuration
    const { user } = await validateUserAndConfig();
    const userId = user.id;
    
    const techStackArray = normalizeTechStack(techStack);

    // 3. Create/Update Project Specification
    const projectSpec = await db.projectSpecification.upsert({
      where: { id: userId }, // Assuming project ID is tied to user ID here based on original logic
      create: {
        userId,
        projectType,
        projectDescription,
    ...
    const uploadedFilesInfo = await handleFileUploads(projectId, files);
    
    // 5. Get Relevant Repositories
    const relevantRepos = await db.relevantRepo.findMany({
      where: { projectId },
      orderBy: { relevanceScore: 'desc' },
      take: 10
    }) as unknown as RelevantRepo[]; // Cast DB result to local interface

    // 6. Generate Build Instructions (LLM Service Call)
    ...
    await db.buildInstructions.upsert({
      ...
      update: { content: instructions, phase: 'setup' }
    });
    ...
    return NextResponse.json({
      message: 'Build instructions generated successfully.',
      instructions,
      projectId,
      filesUploaded: uploadedFilesInfo.length,
      reposFound: relevantRepos.length
    }, { status: 200 });

  } catch (error: any) {
    console.error('Build instructions generation error:', error.message);
    
    // Handle specific validation/config errors (400)
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to generate build instructions due to an internal server error.' },
      { status: 500 }
    );
  }
}