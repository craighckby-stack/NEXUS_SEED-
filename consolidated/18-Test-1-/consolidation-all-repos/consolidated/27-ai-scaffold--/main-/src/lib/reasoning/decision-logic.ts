...
import { EthicalRiskAssessment } from './ethical-scoring';
...
class DecisionEngine {
  ...
  private async logicCheckPhase(task: any, intuitionResult: any): Promise<{
    ...
    const ethicalRisk = await ethicalScoring.calculateERS(task);
  ...
  private async saveDecisionTrace(trace: DecisionTrace): Promise<void> {
    try {
      const response = await fetch('/api/memory/reasoning', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(`Failed to save decision trace: ${response.status} ${response.statusText}`);
      }
    } catch (error) {
