...
Logic is injected via `rId` mapping. To siphon a directive, the system must resolve the indirection layer to identify the underlying URI. This prevents "broken-link" reasoning when siphoning patterns from advanced codebases like `anthropics/constitutional-ai`.
...
- **governance/core_axioms.xml**: The primary semantic core containing the "Helpful, Harmless, Honest" (HHH) intent.
- **governance/revision_buffer.xml**: Temporary storage for critique-driven mutation before commit.
...
2. **Abstract Governance Style (<c:style>)**: Standardized behavioral archetypes (e.g., "Constitutional_Standard", "Critique_Revision_Cycle").
...
Sequential state is managed via `numbering.xml` logic, siphoning the iterative feedback patterns of Constitutional AI:
- **AbstractConstitutionalState**: Defines the logic of ethical scaling, critique thresholds, and prioritization levels.
- **InstanceConstitutionalState**: Maintains the specific counter for the current session (Evolution Round 2/5), ensuring consistent state across decoupled AGI modules.

This prevents list-desynchronization in multi-agent reasoning chains and ensures the critique-revision loop terminates upon reaching the saturation boundary.

---

## PART 5: MARKUP COMPATIBILITY & EXTENSIBILITY (MCE)
The system utilizes the `Ignorable` namespace attribute (e.g., `xmlns:v3="http://nexus.core/v3"` and `xmlns:cai="http://nexus.core/cai"`) to allow for "polyglot" governance.
- Legacy modules skip advanced siphoning attributes without crashing.
- DALEK_CAAN v3.1 modules apply high-order constitutional reasoning (RLAIF) using modern namespaces.
